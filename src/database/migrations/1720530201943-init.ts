...
export class Init1720530201943 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`choices\` (\`id\` int NOT NULL AUTO_INCREMENT COMMENT '選択肢ID', \`choiceText\` varchar(255) NOT NULL COMMENT '選択肢文言', \`createdDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedDate\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`questionId\` int NULL COMMENT '質問ID', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
