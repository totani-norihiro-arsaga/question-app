...
export class CreateQuestionDto{
    @IsString()
    @IsNotEmpty()
    @ApiProperty({type:'string', example:'一番好きなお肉の種類は？', description:'質問文'})
    qustion_text: string;

    @IsEnum(ResponseTypes)
    @ApiProperty({enum:ResponseTypes, example:ResponseTypes.SingleChoice, description:'質問への回答形式'})
    ...
    @IsOptional()
    @IsUUID()
    @ApiProperty({type:'string', example:'ddf04cad-f738-4b85-bd80-2810e96a8b29', description:'アンケートID'})
    survey_id:UUID | null
